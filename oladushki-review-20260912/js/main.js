document.addEventListener('DOMContentLoaded', () => {
  // Shared header; without JS all navigation links remain available.
  const siteHeader = document.querySelector('.lavka-header');
  const menuButton = document.querySelector('.lavka-menu');
  const siteNav = document.querySelector('.lavka-nav');
  if (siteHeader && menuButton && siteNav) {
  ...
  // All products stay in the document; long categories expand on demand.
  document.querySelectorAll('.catalog-main .product-grid').forEach((grid, index) => {
    const cards = [...grid.querySelectorAll('.product-card')];
    if (cards.length <= 4) return;
    grid.id = grid.id || 'product-group-' + index;
    ...
    button.textContent = 'Показать остальные (' + extra.length + ')';
    button.addEventListener('click', () => {
      const open = button.getAttribute('aria-expanded') !== 'true';
      extra.forEach(card => { card.hidden = !open; });
      button.setAttribute('aria-expanded', String(open));
      button.textContent = open ? 'Свернуть раздел' : 'Показать остальные (' + extra.length + ')';
      if (open) {
        const firstAction = extra[0].querySelector('a');
        if (firstAction) firstAction.focus({ preventScroll: true });
      } else {
        grid.previousElementSibling?.scrollIntoView({ block: 'start', behavior: 'auto' });
  ...
  if (orderForm) {
    ...
    if (item) {
      messageArea.value = 'Подскажите цену и наличие: ' + item;
      const map = [
        ['дерев', 'Деревья на заказ'],
        ['изгород', 'Деревья на заказ'],
    ...
    const serviceHints = {
      'Деревья на заказ': 'Напишите желаемую высоту и город. Уточним наличие, стоимость и возможность доставки в переписке.',
...
// ===== Плейлист «Лавка вживую» (28.08.2026) =====
